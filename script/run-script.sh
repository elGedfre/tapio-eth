...
check_config_exists() {
    local chain=$1
    local config_file="$CONFIG_PATH/$chain/factory.json"
...
# Deployment summary
echo "Starting Tapio deployments with version: $VERSION"
...
for chain in "${CHAINS[@]}"; do
    if should_process_chain "$chain"; then
        ...
        [[ "$chain_id" == "Unknown" ]] && { echo "Error: Invalid chain: $chain"; exit 1; }

        if ! check_config_exists "$chain"; then
            echo "Skipping deployment for $chain due to missing config file"
            continue
        fi
        log_dir="./logs/$VERSION/$chain"
        [[ "$DRY_RUN" == "true" ]] && log_dir="$log_dir/dry-run"
        mkdir -p "$log_dir"
        log_file="$log_dir/${SCRIPT_BASE_NAME}_$(date +%Y%m%d_%H%M%S).log"

        # Set environment variables
        env_vars="CHAIN=$chain CHAIN_ID=$chain_id VERSION=$VERSION DRY_RUN=$DRY_RUN CONFIG_PATH=$CONFIG_PATH/$chain"
        if [[ -n "$SAFE_ADDRESS" ]]; then
        ...
        unset CHAIN CHAIN_ID CONFIG_PATH DRY_RUN PRIVATE_KEY SAFE_ADDRESS WALLET_TYPE MNEMONIC_INDEX
    else

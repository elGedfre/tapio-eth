...
describe("GaugeController", function () {
  async function deployTokensAndController() {
    ...
    const GaugeRewardController = await ethers.getContractFactory(
    ...
    const rewardController = await upgrades.deployProxy(GaugeRewardController, [
      rewardToken.address,
      votingEscrow.address,
    ]);
    const controller = await upgrades.deployProxy(GaugeController, [
      rewardToken.address,
      poolToken.address,
      "10000000000",
  ...
  it("should set governance", async () => {
    const { controller, rewardToken, poolToken } = await loadFixture(
      deployTokensAndController,
    );
  ...
  it("should not set governance", async () => {
    ...
    await expect(
      controller.connect(other).proposeGovernance(other.address),
  ...
  it("should update reward rate", async () => {
    ...
    expect(await controller.rewardRatePerWeek()).to.equals(
  ...
  it("should not update reward rate not governance", async () => {
    ...
    await expect(
      controller.connect(other).updateRewardRate("200"),
    ).to.be.revertedWith("not governance");
  });
  ...
  it("should not update reward rate not set", async () => {
    ...
    await expect(controller.updateRewardRate("0")).to.be.revertedWith(
      "reward rate not set",
    );
  });
  ...
  it("should checkpoint", async () => {
    ...
    await rewardToken
      .connect(other)
      .approve(votingEscrow.address, "1000000000000000000");
    await votingEscrow
      .connect(other)
      .createLock("1000000000000000000", 1753675829);
    await rewardController
      .connect(other)
      .voteForGaugeWeights(poolOne.address, "1000");
    await rewardController
      .connect(other)
      .voteForGaugeWeights(poolTwo.address, "1000");
